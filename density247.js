...
var buckets = 11,
	...
	stateAbbrs = ['all', '131', '130', '85', '134', '133', '132', '155', '139', '140', '23', '145', '148', '149', '147', '144', '125', '138', '153', '154', '150', '151', '152'],
	states = {
		all: { name: 'All Buildings', abbr: 'all', offset: 0 },
		B131: { name: 'Butler 3', abbr: '131', offset: -1 },
		B130: { name: 'Butler 2', abbr: '130', offset: 2 },
		L85: { name: 'Roone Arledge', abbr: '85', offset: 2 },
		B134: { name: 'Butler 6', abbr: '134', offset: 0 },
		B133: { name: 'Butler 5', abbr: '133', offset: 0 },
		J155: { name: "JJ's Place", abbr: '155', offset: 1 },
		L139: { name: 'Lehman 2', abbr: '139', offset: 3 },
		L140: { name: 'Lehman 3', abbr: '140', offset: 3 },
		U23: { name: 'Uris', abbr: '23', offset: 3 },
		S145: { name: 'Sci & Eng Library', abbr: '145', offset: 3 },
		A148: { name: 'Avery 2', abbr: '148', offset: 3 },
		A149: { name: 'Avery 3', abbr: '149', offset: -3 },
		A147: { name: 'Avery 1', abbr: '147', offset: 2 },
		S144: { name: 'Starr East Asian', abbr: '144', offset: 1 },
		J125: { name: 'John Jay Dining Hall', abbr: '125', offset: 2 },
		B138: { name: 'Butler Stacks', abbr: '138', offset: 3 },
		L153: { name: 'Lerner 4', abbr: '153', offset: 2 },
		L154: { name: 'Lerner 5', abbr: '154', offset: 3 },
		L150: { name: 'Lerner 1', abbr: '150', offset: 2 },
		L151: { name: 'Lerner 2', abbr: '151', offset: 3 },
		L152: { name: 'Lerner 3', abbr: '152', offset: 3 }
	};
...
function getCalcs(state, view) {
	...
	for (var d = 0; d < data[state].views.length; d++) {
		for (var h = 0; h < data[state].views[d].length; h++) {
			...
			if (view === 'all') {
				var tot = data[state].views[d][h].sep + data[state].views[d][h].oct;
			} else {
	...
	for (var d = 0; d < data[state].views.length; d++) {
		for (var h = 0; h < data[state].views[d].length; h++) {
			...
			var sel = '#d' + d + 'h' + h + ' .tile .' + side,
				val = data[state].views[d][h].sep + data[state].views[d][h].oct;
...
function drawHourlyChart(state, day) {
	...
	var y = d3.scale.linear()
		.domain([0, d3.max(weeklyData, function(d) { return (view === 'all') ? d.sep + d.oct : d[view] })])
		.range([0, h]);
	...
	rect.data(weeklyData)
		.enter()
			.append('svg:rect')
				...
				.attr('y', function(d) { return (view === 'all') ? h - y(d.sep + d.oct) : h - y(d[view]) })
				.attr('height', function(d) { return (view === 'all') ? y(d.sep + d.oct) : y(d[view]) })
				.attr('width', 10)
